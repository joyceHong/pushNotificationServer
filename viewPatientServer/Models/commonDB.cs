...
namespace viewPatientServer
{
    public  class commonDB
    {
      ...
      /// <param name="dt">資料集</param>
        ...
        /// <returns></returns>
        protected static DataTable selectQueryWithDataTable(string strCommand)
        {
           return selectQueryWithDataTable(strCommand, false);
        }


        protected static DataTable selectQueryWithDataTable(string strCommand, bool isShowDeleted)
        {
        ...
        public  static List<T> selectQueryWithListT<T>(string strCommand, bool isShowDeleted) where T:new()
        {
            try
            {
                ...
                {
                    T valueObj = new T();
                    Type myType = valueObj.GetType();
                    IList<PropertyInfo> props = new List<PropertyInfo>(myType.GetProperties());
                    int i = 0;
                    while (oledbReaderObj.Read())
                    {
                        T newData = new T();
                        //Type newData = Type.GetType(T);  
                        foreach (PropertyInfo prop in props)
                        {
                            if (prop.Name == "strDoctorIDAndName")
                            {
                                prop.SetValue(newData, oledbReaderObj[0] + " " + oledbReaderObj[1]);
                            }
                            else
                            {
                                if (i <= oledbReaderObj.VisibleFieldCount-1)
                                {
                                    prop.SetValue(newData, oledbReaderObj[i]);
                                }
                                i++;
                            }
                        }
